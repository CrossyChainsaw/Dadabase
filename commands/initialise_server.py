...
from Dadabase.modules.command import LEADERBOARD_COMMUNITY_EDIT_DATA
from Dadabase.classes.Server import Server

async def leaderboard_community_init(interaction, leaderboard_title, sorting_method, show_member_count, show_no_elo_players, channel_1v1_id, channel_2v2_id, channel_rotating_id, color, image, flag_type):
    # Logic
    ...
    if os.path.exists(f"{SERVERS_DATA_PATH}{interaction.guild.id}.json"):
        await interaction.response.send_message(f"Oops! This server already exists. Consider running `{LEADERBOARD_COMMUNITY_EDIT_DATA.name}` to update data.")
    else:
