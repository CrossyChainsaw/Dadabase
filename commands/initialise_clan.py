...
from Dadabase.modules.command import LEADERBOARD_CLAN_EDIT_DATA
from Dadabase.classes.Clan import Clan

async def leaderboard_clan_init(interaction, clan_names: str, channel_1v1_id:str, channel_2v2_id:str, clan_id:str, color:str, image:str, sorting_method:str, show_member_count:bool, show_xp:bool, show_no_elo_players:bool, channel_rotating_id:str, server_id=None, server_name=None):
    # Convert Fields
    ...
    if os.path.exists(f"{CLANS_DATA_PATH}{server_id}.json"):
        await interaction.response.send_message(f"Oops! This server already exists. Consider running `{LEADERBOARD_CLAN_EDIT_DATA.name}` to update data.")
    else:
